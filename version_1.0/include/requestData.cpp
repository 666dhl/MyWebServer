...
requestData::requestData(): // requestData类默认构造函数
...
requestData::requestData(int _epollfd, int _fd, std::string _path):// requestData类有参构造函数
    now_read_pos(0), state(STATE_PARSE_URI), h_state(h_start), 
    keep_alive(false), againTimes(0), timer(NULL),
    path(_path), fd(_fd), epollfd(_epollfd)
{}

requestData::~requestData() {// requestData类析构函数
...
int requestData::getFd() {// 当前requestData
    return fd;
}
...
void requestData::handleRequest() {
    ...
    while (true) {
        ...
        if (read_num < 0) {
            ...
            // 最可能是对端已经关闭了，统一按照对端已经关闭处理 error_ = true;
            perror("read_num == 0");
            if (errno == EAGAIN) {
                if (againTimes > AGAIN_MAX_TIMES) isError = true;
                else ++againTimes;
            }
            else if (errno != 0) isError = true;
            break;
        }
        ...
        if (state == STATE_RECV_BODY) {
            ...
            if (content.size() < content_length) continue;// 到达的报文内容长度未完全，继续等待
            state = STATE_ANALYSIS;// 开始分析
        }
    ...
    int ret = epoll_mod(epollfd, fd, static_cast<void*>(this), _epo_event); // 修改epoll中的描述符状态
    if (ret < 0) {
        // 返回错误处理
        delete this;
    }
...
int requestData::parse_URI() {
    ...
    if (str.size() > pos + 1)  str = str.substr(pos + 1);// 去掉 请求行
    else str.clear();
    ...
    if (pos < 0) {
        ...
        if (pos < 0) {
            return PARSE_URI_ERROR;// 既不是 GET 也不是 POST
        } else {
    ...
    pos = request_line.find("/", pos);// 从 GET 或 POST 后面开始找
    ...
    if (pos < 0) {
        ...
        if (_pos < 0) return PARSE_URI_ERROR;
            ...
            } else file_name = "../include/index.html";// 如果没有给出文件名，默认使用这个
        }
        pos = _pos;
    }
...
int requestData::parse_Headers() {
    string &str = content;
    int key_start = -1, key_end = -1;// 键开始的位置，键结束的位置
    int value_start = -1, value_end = -1;// 值开始的位置，值结束的位置
    int now_read_line_begin = 0;// 当前行的开始在整个首部行的位置
    bool notFinish = true;

    for (int i = 0; i < str.size() && notFinish; ++i) {
        // 下面是一个状态机 依次读取key value
        switch(h_state) {
            // h_state初始化中是被初始化为h_start的
            case h_start: {
                //开始
                if (str[i] == '\n' || str[i] == '\r') break;
                h_state = h_key;
            ...
            case h_value: {
                ...
                } else if (i - value_start > 255) return PARSE_HEADER_ERROR;// value 过长
                break;  
            }
            case h_CR: {
                if (str[i] == '\n') {
    ...
    if (h_state == h_end_LF) {
        str = str.substr(now_read_line_begin);//把首部行结束处到报文结束处（实体部分）作为子串
...
int requestData::analysisRequest() {
    if (method == METHOD_POST) {
        //get content
        char header[MAX_BUFF];//header存放回送报文 后面的headers存放刚才读取的首部行key value对
        ...
        if (dot_pos < 0) filetype = MimeType::getMime("default").c_str();
        else filetype = MimeType::getMime(file_name.substr(dot_pos)).c_str();
...
void requestData::handleError(int fd, int err_num, string short_msg) {
    ...
    body_buff += "<hr><em> Dong's Web Server</em>\n</body></html>";
...
void mytimer::clearReq() {// 清空
    request_data = NULL;
    this->setDeleted();
}

void mytimer::setDeleted() {// 设置计时器为关闭状态
    deleted = true;
}

bool mytimer::isDeleted() const {// 检查当前计时器是否处于关闭状态
    return deleted;
}

size_t mytimer::getExpTime() const {// 获得过期时间
    return expired_time;
}
