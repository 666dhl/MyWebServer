...
const int THREADPOOL_THREAD_NUM = 4;// 线程池中线程数量
const int QUEUE_SIZE = 65535;// 任务队列数量
...
extern pthread_mutex_t qlock;// 互斥锁
...
void myHandler(void *args) {
    //因为在mian函数一开始epoll事件结构体的event.data.ptr 项就是用requestData转换过去的 所以这里可以转换回来
...
void handle_events(int epoll_fd, int listen_fd, struct epoll_event* events, int events_num, const string &path, threadpool_t* tp) {
    for(int i = 0; i < events_num; i++) {
        ...
        if(fd == listen_fd) {
            ...
            //myHandler是对任务的处理函数   events[i].data.ptr是用户传过来的数据(报文) 作为任务处理函数的参数
...
(2) 第二个好处是给超时时间一个容忍的时间，就是设定的超时时间是删除的下限(并不是一到超时时间就立即删除)，如果监听的请求在超时后的下一次请求中
又一次出现了，就不用再重新申请requestData节点了，这样可以继续重复利用前面的requestData，减少了一次delete和一次new的时间。
*/
...
int main() {
    /******设置信号SIGPIPE的处理操作*******/
    //防止客户端进程终止，而导致服务器进程被SIGPIPE信号终止
    ...
    while (true) {
       ...
       /******就绪的事件放入events事件结构体数组*******/
        ...
        /******处理外部io事件*******/
        //epoll_fd表示epoll事件表的套接字  listenfd表示监听套接字的描述符  events_num表示就绪io套接字的数组 
